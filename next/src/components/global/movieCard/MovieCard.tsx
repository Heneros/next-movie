...
export default function MovieCard({ info }: { info: MovieItem }) {
    ...
    return (
            ...
            {isLoading ? (<Skeleton height={250} />) : previewData ? (
                <>
                    <div className='group relative'>
                        ...
                        <div
                            className=" top-0 left-0 z-10 w-14 h-14 rounded-2xl backdrop-blur-lg flex justify-center items-center
        ...
        absolute  duration-500 ease-in-out"
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 rounded-2xl transition-all duration-300 z-20 " />
                        <div className="absolute inset-0 p-6 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300">


                            <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 z-30  ">
                                ...
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div className='grid grid-cols-2 gap-2'>
                                        {info.category.map((item) => (
                                            <span className="text-gray-300 text-sm bg-gray-800/50 px-1 py-1 rounded">{item} </span>
                                        ))}
                                    </div>
                                </div>
